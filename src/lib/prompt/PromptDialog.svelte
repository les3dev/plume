...
{#if creating}
    <div class="flex min-w-lg flex-col gap-6">
        ...
        <div class="flex flex-col gap-4">
            ...
            <input type="text" id="new-title" bind:value={new_prompt.title} />
        </div>

        <div class="flex flex-col gap-2">
            <label for="new-prompt">Prompt</label>
            <textarea id="new-prompt" bind:value={new_prompt.prompt}></textarea>
        </div>
...
{:else if editing_prompt !== null}
    <div class="flex min-w-lg flex-col gap-6">
        ...
        <div class="flex flex-col gap-2">
            ...
            <textarea id="prompt" bind:value={editing_prompt.prompt}></textarea>
        </div>
...
{:else}
    <div class="flex min-w-lg flex-col gap-6">
        <div class="flex items-center">
            ...
            />
            <button class="btn" onclick={() => (creating = true)}>
        ...
        {#each prompt_context.prompts.filter((p) => !tabs.some((t) => t.id === p.id)) as prompt (prompt.id)}
            <div class="flex items-center">
                ...
                <button class="btn ghost" onclick={() => (editing_prompt = {...prompt})}>
                    <PenIcon --size="1rem" />
                </button>
            </div>
        {/each}
