...
use crate::{audio::audio_buffer::AudioBuffer, capture_state::{CaptureState, SafeStream}};
...
pub fn start_capture(state: State<CaptureState>) -> Result<(), String> {
    ...
    *state.mic_buf.lock().unwrap() = AudioBuffer::default();
    ...
    engine.start_capture(move |samples, _frames, sample_rate, channels| {
        sys_buf.lock().unwrap().push(samples, sample_rate as u32, channels as u16);
    }).map_err(|e| e.to_string())?;
    *state.sys_engine.lock().unwrap() = Some(engine);

    // ── Mic Audio ──
    let host = cpal::default_host();
    let mic_device = host.default_input_device().ok_or("No default input device")?;
    let config = mic_device.default_input_config().map_err(|e| e.to_string())?;
    ...
    let stream = match config.sample_format() {
        cpal::SampleFormat::F32 => {
            mic_device.build_input_stream(&config.into(), move |data: &[f32], _| {
